...
public final class AgentConnectIdentityProviderFactory
  ...
  public List<ProviderConfigProperty> getConfigProperties() {
    ...
    return ProviderConfigurationBuilder.create()
        ...
        .defaultValue(MfaRequirement.DISABLED.name())
        .add()
        .property().name(EidasLevel.EIDAS_LEVEL_PROPERTY_NAME)
        .label("Niveau de garantie eIDAS")
        .helpText("Permet de fixer le niveau de garantie minimum du compte utilisateur. En mode 2FA, filtre les valeurs ACR MFA envoyées dans le paramètre claims. Effet : désactive des fournisseurs d'identités (FI) sur la page de login AgentConnect.")
        .type(ProviderConfigProperty.LIST_TYPE)
        .options(eidasLevels)
